...
public class CountTable implements Serializable {
	 ...
	 * @param fullSeqLength         Size of the sequence before calling recursive
	 *                              divider
	...
	public void push(Integer key, int curSeqLength, int fullSeqLength, int numberOfSeqSameLength, int dist) {
		...
		// float weightLength = (float)curSeqLength / fullSeqLength; //from [1.0,0[ ->
		// higher is better
		// float weightLength = (float)fullSeqLength / curSeqLength; //from [1.0,0[ ->
		// higher is better

		// calculate the value for the current key
		// float curValue = (weightLevel * 0.5f) + (weightLength * 5.0f) +
		// (weightDistance * 1.8f);
		// float curValue = (weightLevel * 1f) + (weightLength * 1f) + (weightDistance *
		// 0.0001f);
	...
	/**
	 * Update this CountTable with a sequence S, it finds the similar sequence SS of
	 * S
	 ...
	 * @param initialSequenceSize The initial size of the sequence to predict (used
	 *                            for weighting)
	 */
	...
	public Sequence customeGetBestSequence(int count) {
		...
		List<Integer> bestItems = sd.customeGetBest(1.002,count);
		// List<Integer> bestItems = sd.customeGetBest(1.00001);
	 ...
	 * @return The sequence containing the |count| best items sorted from the
	 *         CountTable
	 */
	public Sequence getBestSequence(int count) {
		...
		for (Entry<Integer, Float> it : table.entrySet()) {
			...
			// the following measure of confidence and lift are "simplified" but are exactly
