...
public class ScoreDistribution<K> {
	...
	public List<K> customeGetBest(double minThreshold, int count) {
		if (dict.size() == 0) {
			return null;
		} else if (dict.size() == 1) {
		...
		if ((bestVal1 / bestVal2) < minThreshold) {
			...
			// Uncomment this to get the top 5 items
			int i;
			List<K> temp = dict.get(bestVal1);
			String results = "Symbol " + temp + " with score: " + bestVal1 + "\n";
			Double[] bestVals = new Double[count];
			List<K> bestOfTheBestItems = temp;
			bestVals[0] = bestVal1;
			for (i = 1; i < count; i++) {
				bestVals[i] = dict.lowerKey(bestVals[i - 1]);
				temp = dict.get(bestVals[i]);
				bestOfTheBestItems.addAll(temp);
				results += "Symbol " + temp + " with score: " + bestVals[i] + "\n";
			}
			System.out.println("The score of top " + count + " items are: ");
			System.out.println(results);
			return bestOfTheBestItems;
		}
	}
	 ...
	 * 
	 * @param minThreshold Min ratio between the best and second best value
	 *                     [1.0,0.0]
	...
	public List<K> getBest(double minThreshold) {
		...
		if ((bestVal1 / bestVal2) < minThreshold) {
			...
			// Uncomment this to get performance comparison run properly
